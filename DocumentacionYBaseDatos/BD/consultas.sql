...
/*Obtener precio costo de los muebles vendidos en determinada fecha*/
SELECT SUM(precio_costo) AS Costo FROM Mueble WHERE EXISTS (SELECT C_idMueble FROM Compra WHERE Compra.C_idMueble = Mueble.idMueble AND Cfecha between "2021-09-01" and "2021-10-01");